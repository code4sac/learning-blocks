...
def get_current_grades(
  ...
  header =['StID','SchYear','SchID','Term','Abs_ex','Abs_unex','DaysAtt','DaysEnr','DaysSuspension','DaysTardy']
  ...
  with open(file_path, 'w', newline='') as current_grades_csv_file:
    ...
    for x in range(len(c['HistorySummaries'])):
      csv_writer.writerow([f'{student_ids}',c['HistorySummaries'][x]['SchoolYear'],c['HistorySummaries'][x]['SchoolCode'],"Please Define term",c['HistorySummaries'][x]['DaysExcused'],c['HistorySummaries'][x]['DaysUnexcused'],c['HistorySummaries'][x]['DaysPresent'],c['HistorySummaries'][x]['DaysEnrolled'],c['HistorySummaries'][x]['DaysSuspension'],c['HistorySummaries'][x]['DaysTardy']])
