...
async def read_people(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    people = await asyncio.to_thread(lambda: db.query(models.PeopleInDB).offset(skip).limit(limit).all())
...
async def create_person(person: schemas.PeopleInDBSchema, db: Session = Depends(get_db)):
    """
...
async def update_person(person_id: str, person: schemas.PeopleInDBSchema, db: Session = Depends(get_db)):
    """
    Update a person asynchronously.
    """
    db_person = await asyncio.to_thread(lambda: db.query(models.PeopleInDB).filter(models.PeopleInDB.AnonymizedStudentID == person_id).first())
    if not db_person:
...
async def read_person(person_id: str, db: Session = Depends(get_db)):
    """
