...
def get_transcripts(
    ...
    with open('All_Course_Grades_in_Transcripts.json') as json_file:
        ...
        with open(file_path, 'w', newline='',) as enrollment_csv_file:
            header =['Id','SchYear','SchID','Term','ClassN','Units','GradeAch','Class','ClassT','ClassLvl_1/2','ClassLvl_2/2','A_Gstatus_1/2','A_Gstatus_2/2',]
            csv_writer = csv.writer(enrollment_csv_file)
            csv_writer.writerow(header)

            for manydict in range(len(requesttool[0]['CourseGrades'])):
                c= dict(a[manydict])
                csv_writer.writerow([student_ids,c['SchoolYear'],c['SchoolTakenName'],c['Term'],c['CourseID'],c['CreditCompleted'],c['Mark'],c['CourseID']])
 # End: Create CSV File
...
get_transcripts('https://demo.aeries.net/aeries/','477abe9e7d27439681d62f4e0de1f5e1','994','99400002')
