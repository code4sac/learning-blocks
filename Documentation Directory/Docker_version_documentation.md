...
In that case, you will need to use a fake local domain (`dev.learning-blocks.com`) and make your computer think that the
domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.learning-blocks.com` was configured to be allowed. If you want a
...
* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character,
  and your fake local domain: `dev.learning-blocks.com`.
...
to `dev.learning-blocks.com` and think that it is a remote server while it is actually running in your computer.

To configure it in your stack, follow the section **Change the development "domain"** below, using the
domain `dev.learning-blocks.com`.

After performing those steps you should be able to open: http://dev.learning-blocks.com and it will be server by your
stack in `localhost`.
...
Docker Toolbox with `local.dockertoolbox.tiangolo.com`, using `localhost.tiangolo.com` or
using `dev.learning-blocks.com`).
