    ...
    with open(file_path, 'w', newline='') as Transcripts_2_of_2_csv_file:
        ...
        for x in range(Courseslen):
            ...
            for manydict in range(len(a)):
                c = dict(a[manydict])
            csv_writer.writerow(
                [c['ID'], c['LongDescription'], c['NonAcademicOrHonorsCode'], c['CollegePrepIndicatorCode'],
                 c['CSU_SubjectAreaCode'], c['UC_SubjectAreaCode']])
            print(c.keys())
            print(c.values())
...
get_transcripts2('https://demo.aeries.net/aeries', '477abe9e7d27439681d62f4e0de1f5e1', '990', '99400002',
                 courses=['insert list of courses'])
